<script setup lang="ts">
        ...
        const videoId = extractYouTubeId(videos.value[index].videoUrl);
        ...
        async function getYouTubeTitle(videoUrl: string) {
        const videoId = extractYouTubeId(videoUrl);
        if (!videoId) return 'Invalid Video URL';
        ...
        try {
            ...
            return data.title; // Extract the title
        } catch (error) {
        ...
        // Update Titles on Mount
        async function updateVideoTitles() {
        for (let i = 0; i < videos.value.length; i++) {
            videos.value[i].title = await getYouTubeTitle(videos.value[i].videoUrl);
        }
        }
